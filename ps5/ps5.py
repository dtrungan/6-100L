...
def filter(pixels_list, color):
    ...
    for pixel in pixels_list:
        pixel = matrix_multiply(filter_matrix, list(pixel))
        pixel = tuple(int(e) for e in pixel)
        transformed_list.append(pixel)
...
def extract_end_bits(num_end_bits, pixel):
    ...
    return tuple(e % 2**num_end_bits for e in pixel)
...
def reveal_bw_image(filename):
    ...
    for pixel in pixels:
        hidden_pixels.append(extract_end_bits(1, pixel) * 255)

    hidden_im = Image.new("L", im.size)
    hidden_im.putdata(hidden_pixels)
    return hidden_im
...
def reveal_color_image(filename):
    ...
    for pixel in pixels:
        scaled = tuple(int(e * 255 / 7) for e in extract_end_bits(3, pixel))
        hidden_pixels.append(scaled)

    hidden_im = Image.new("RGB", im.size)
    hidden_im.putdata(hidden_pixels)
    return hidden_im
